...
app = FastAPI(title="RNUdb API", description="Simple read-only API for RNAdb data")
...
async def get_gene_structure(gene_id: str):
    ...
    try:
        ...
        # Get nucleotides
        cursor.execute("SELECT * FROM nucleotides WHERE structure_id = ? ORDER BY id", (structure_id,))
        ...
        # Get base pairs
        cursor.execute("SELECT * FROM base_pairs WHERE structure_id = ?", (structure_id,))
        base_pair_rows = cursor.fetchall()
        
        base_pairs = []
        for row in base_pair_rows:
            row_dict = dict(row)
            base_pairs.append(BasePair(from_=row_dict['from_pos'], to=row_dict['to_pos']))
        
        # Get annotations
        cursor.execute("SELECT * FROM annotations WHERE structure_id = ?", (structure_id,))
