...
VALID_CLINICAL_SIG = {
    ...
    "Likely Benign", "Benign", "LP", "LB", "P", "B"
}
...
def validate_variant_batch(
    ...
    for i, row in enumerate(rows, start=1):
        ...
        if position is None:
            row_errors.append(ValidationError(i, "position", "Position is required", None))
        if not ref:
            row_errors.append(ValidationError(i, "ref", "Reference allele is required", ref))
        if not alt:
            row_errors.append(ValidationError(i, "alt", "Alternate allele is required", alt))
        ...
        try:
            ...
            row_errors.append(ValidationError(i, "position", "Position must be a number", position))
            errors.extend(row_errors)
            continue
        ...
        if ref and not all(c in VALID_NUCLEOTIDES for c in str(ref)):
            row_errors.append(ValidationError(
                i, "ref",
                f"Invalid reference allele '{ref}'. Must be valid IUPAC nucleotides",
                ref
            ))
        ...
        if alt and not all(c in VALID_NUCLEOTIDES for c in str(alt)):
            row_errors.append(ValidationError(
                i, "alt",
                f"Invalid alternate allele '{alt}'. Must be valid IUPAC nucleotides",
                alt
            ))
        ...
        if zygosity and zygosity.lower() not in {"het", "hom", "heterozygous", "homozygous"}:
        ...
        numeric_fields = ["function_score", "cadd_score", "gnomad_ac", "gnomad_hom",
        ...
        hgvs = row.get("hgvs")
        if hgvs and not re.match(r"^(c\.|n\.|g\.|m\.|r\.)[\.\d\w>\-_\*\(\)\+\?]+$", str(hgvs)):
            row_warnings.append(ValidationError(
                i, "hgvs",
                f"HGVS notation '{hgvs}' may be malformed. Expected format: c.123A>G or n.140G>A",
                hgvs
            ))
        ...
        if variant_key in existing_variants:
            row_warnings.append(ValidationError(
                i, "duplicate",
                f"Variant at position {pos} with ref={ref}, alt={alt} already exists in database",
                variant_key
            ))
        ...
        if row_errors:
            ...
            if clinical_sig:
                normalized_row["clinical_significance"] = CLINICAL_SIG_MAP.get(clinical_sig, clinical_sig)
...
def validate_structure(
    ...
    if not isinstance(nucleotides, list):
        ...
        errors.append(ValidationError(0, "nucleotides", "At least one nucleotide is required"))
    
    base_pairs = data.get("basePairs")
    if not isinstance(base_pairs, list):
        errors.append(ValidationError(0, "basePairs", "basePairs must be an array"))
    ...
    for i, nt in enumerate(nucleotides, start=1):
        if not isinstance(nt, dict):
            errors.append(ValidationError(i, "nucleotide", "Each nucleotide must be an object", nt))
            continue
        
        nt_id = nt.get("id")
        if not isinstance(nt_id, int):
            errors.append(ValidationError(i, "nucleotide.id", "Nucleotide ID must be an integer", nt_id))
        else:
    ...
    for i, bp in enumerate(base_pairs, start=1):
        if not isinstance(bp, dict):
            errors.append(ValidationError(i, "basePair", "Each base pair must be an object", bp))
            continue
        ...
        if not isinstance(from_pos, int):
            errors.append(ValidationError(i, "basePair.from", "from must be an integer", from_pos))
        ...
        if not isinstance(to_pos, int):
            errors.append(ValidationError(i, "basePair.to", "to must be an integer", to_pos))
    ...
    features = data.get("structuralFeatures", [])
    if isinstance(features, list):
        for i, feature in enumerate(features, start=1):
            if not isinstance(feature, dict):
                errors.append(ValidationError(i, "structuralFeature", "Each feature must be an object"))
                continue
            
            if not feature.get("id"):
                errors.append(ValidationError(i, "structuralFeature.id", "Feature ID is required"))
            if not feature.get("featureType"):
                errors.append(ValidationError(i, "structuralFeature.featureType", "Feature type is required"))
            
            nt_ids = feature.get("nucleotideIds", [])
            if not isinstance(nt_ids, list):
                errors.append(ValidationError(i, "structuralFeature.nucleotideIds", "nucleotideIds must be an array"))
            else:
                for nt_id in nt_ids:
                    if nt_id not in valid_nucleotide_ids:
                        errors.append(ValidationError(
                            i, "structuralFeature.nucleotideIds",
            ...
            if not isinstance(label, dict) or not label.get("text"):
                errors.append(ValidationError(i, "structuralFeature.label", "Feature label with text is required"))
    ...
    if gene_seq and len(nucleotides) != len(gene_seq):
        warnings.append(ValidationError(
            0, "nucleotide_count",
            f"Nucleotide count ({len(nucleotides)}) does not match gene sequence length ({len(gene_seq)})",
...
def validate_bed_intervals(
    ...
    for i, interval in enumerate(intervals, start=1):
        ...
        if chrom_start is None:
            row_errors.append(ValidationError(i, "chromStart", "Start coordinate is required"))
        if chrom_end is None:
            row_errors.append(ValidationError(i, "chromEnd", "End coordinate is required"))
        ...
        try:
            ...
            row_errors.append(ValidationError(i, "chromStart", "Start must be an integer", chrom_start))
            start = None
        
        try:
            end = int(chrom_end)
        except (ValueError, TypeError):
            row_errors.append(ValidationError(i, "chromEnd", "End must be an integer", chrom_end))
            end = None
        ...
        if start is not None and end is not None:
            ...
            if chrom_normalized != gene_chr:
                row_errors.append(ValidationError(
                    i, "chrom",
                    f"Chromosome '{chrom}' does not match gene chromosome '{gene.get('chromosome')}'",
                    chrom
                ))
            ...
            if start < gene_start or end > gene_end:
                row_errors.append(ValidationError(
                    i, "coordinates",
                    f"Interval ({start}-{end}) is outside gene bounds ({gene_start}-{gene_end})",
                    {"start": start, "end": end}
                ))
        ...
        if score is not None and score != "":
            try:
                ...
                row_errors.append(ValidationError(
                    i, "score",
                    f"Score must be numeric, got '{score}'",
                    score
                ))
