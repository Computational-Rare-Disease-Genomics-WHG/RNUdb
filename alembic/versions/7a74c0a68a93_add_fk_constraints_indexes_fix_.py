...
def upgrade() -> None:
    ...
    with op.batch_alter_table('annotations') as batch_op:
        batch_op.create_foreign_key('fk_annotations_structure_id', 'rna_structures', ['structure_id'], ['id'])

    with op.batch_alter_table('base_pairs') as batch_op:
        batch_op.create_foreign_key('fk_base_pairs_structure_id', 'rna_structures', ['structure_id'], ['id'])
    ...
    with op.batch_alter_table('nucleotides') as batch_op:
        batch_op.create_foreign_key('fk_nucleotides_structure_id', 'rna_structures', ['structure_id'], ['id'])

    with op.batch_alter_table('pending_changes') as batch_op:
        batch_op.create_index('ix_pending_changes_requested_by', ['requested_by'], unique=False)

    with op.batch_alter_table('rna_structures') as batch_op:
        batch_op.create_index('ix_rna_structures_geneId', ['geneId'], unique=False)
        batch_op.create_foreign_key('fk_rna_structures_geneId', 'genes', ['geneId'], ['id'])

    with op.batch_alter_table('structural_features') as batch_op:
        batch_op.create_foreign_key('fk_structural_features_structure_id', 'rna_structures', ['structure_id'], ['id'])

    with op.batch_alter_table('variant_classifications') as batch_op:
        batch_op.create_foreign_key('fk_variant_classifications_variant_id', 'variants', ['variant_id'], ['id'])
        batch_op.create_foreign_key('fk_variant_classifications_literature_id', 'literature', ['literature_id'], ['id'])

    with op.batch_alter_table('variant_links') as batch_op:
        batch_op.create_foreign_key('fk_variant_links_variant_id_2', 'variants', ['variant_id_2'], ['id'])
        batch_op.create_foreign_key('fk_variant_links_variant_id_1', 'variants', ['variant_id_1'], ['id'])
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('variant_classifications') as batch_op:
        batch_op.drop_constraint('fk_variant_classifications_literature_id', type_='foreignkey')
        batch_op.drop_constraint('fk_variant_classifications_variant_id', type_='foreignkey')

    with op.batch_alter_table('structural_features') as batch_op:
        batch_op.drop_constraint('fk_structural_features_structure_id', type_='foreignkey')
