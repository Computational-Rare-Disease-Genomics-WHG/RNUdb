...
export const useImportExport = () => {
  ...
  const downloadJSON = useCallback(
    (rnaData: RNAData, filename?: string) => {
      const dataStr = exportToJSON(rnaData);
      const dataUri =
        "data:application/json;charset=utf-8," + encodeURIComponent(dataStr);
      const exportFileDefaultName =
        filename ||
        `${rnaData.name.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.json`;
  ...
  const importFromJSON = useCallback((jsonString: string): RNAData | null => {
    try {
      ...
      if (parsed.structural_features) {
        for (const feature of parsed.structural_features) {
          if (
            !feature.id ||
            !feature.feature_type ||
            !Array.isArray(feature.nucleotide_ids)
          ) {
            throw new Error("Invalid structural feature structure");
          }
          if (!feature.label_text || typeof feature.label_text !== "string") {
            throw new Error("Invalid structural feature label_text");
          }
        }
      }
      ...
      return {
        ...
        base_pairs: parsed.base_pairs || parsed.basePairs,
  ...
  const listSavedStructures = useCallback((): Array<{
    key: string;
    name: string;
    savedAt: string;
  }> => {
