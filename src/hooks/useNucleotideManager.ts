...
export const useNucleotideManager = (initialData: RNAData) => {
  ...
  const getSmartSnapPosition = useCallback(
    (x: number, y: number) => {
  ...
  const addNucleotide = useCallback(
    (x: number, y: number) => {
  ...
  const updateNucleotideId = useCallback(
    (oldId: number, newId: number) => {
      ...
      setRnaData((prev) => ({
        ...
        base_pairs: prev.base_pairs.map((bp) => ({
          from_pos: bp.from_pos === oldId ? newId : bp.from_pos,
          to_pos: bp.to_pos === oldId ? newId : bp.to_pos,
        })),
      }));
  ...
  const addBasePair = useCallback(
    (nucleotide1: number, nucleotide2: number) => {
      const exists = rnaData.base_pairs.some(
        (bp) =>
  ...
  const updateRnaData = useCallback(
    (newData: RNAData) => {
      setRnaData(newData);
      setCurrentNucleotide(null);
      setSelectedNucleotides([]);
  ...
  const updateStructuralFeature = useCallback(
    (featureId: string, updates: Partial<StructuralFeature>) => {
      setRnaData((prev) => ({
        ...prev,
        structural_features: prev.structural_features?.map((f) =>
