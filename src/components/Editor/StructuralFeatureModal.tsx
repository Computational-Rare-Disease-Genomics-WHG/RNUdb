...
export const StructuralFeatureModal: React.FC<StructuralFeatureModalProps> = ({
  ...
  useEffect(() => {
    if (selectedNucleotides.length > 0 && !initialFeature) {
      const selectedNucs = nucleotides.filter((n) =>
  ...
  useEffect(() => {
    if (initialFeature) {
      setFeatureType(initialFeature.feature_type);
      setLabelText(initialFeature.label_text);
      setLabelX(initialFeature.label_x);
      setLabelY(initialFeature.label_y);
      setFontSize(initialFeature.label_font_size);
      setLabelColor(initialFeature.label_color || "#7c3aed");
