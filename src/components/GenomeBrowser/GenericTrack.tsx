...
const GenericTrack: React.FC<GenericTrackProps> = ({
  title,
  height,
  data,
  regions,
    ...
    <Track title={title}>
      {({ scalePosition, width }) => {
        ...
        const dataPoints = Object.entries(data)
          ...
          .filter(
            (point) =>
              point.genomicPos >= currentRegion.start &&
              point.genomicPos <= currentRegion.stop,
          )
            ...
            {displayType === "line" ? (
                ...
                {dataPoints.length > 1 && (
                  <polyline
                    points={dataPoints
                      .map(
                        (point) =>
                          `${scalePosition(point.genomicPos)},${scaleY(point.value)}`,
                      )
                      .join(" ")}
                    fill="none"
                    stroke="#374151"
              ...
              dataPoints.map((point) => {
                const barHeight = Math.abs(
                  scaleY(point.value) - scaleY(Math.max(0, minValue)),
                );
                const barY =
                  minValue < 0 && point.value < 0
