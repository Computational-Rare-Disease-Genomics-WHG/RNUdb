...
import { Database, Dna, Globe, Search, AlertTriangle, Users, ExternalLink, Link as LinkIcon } from 'lucide-react';
...
const InfoPanel: React.FC<InfoPanelProps> = ({
  ...
  const getVariantInfoForNucleotide = (nucleotideId: number) => {
    ...
    return { clinicalVariants, populationVariants, allVariants: filteredVariants };
  };
  ...
  const getClinicalBadgeClass = (significance?: string) => {
    ...
    if (lower.includes('pathogenic') && !lower.includes('likely')) return 'bg-red-100 text-red-700 border-red-200';
    if (lower.includes('likely pathogenic') || lower === 'lp') return 'bg-orange-100 text-orange-700 border-orange-200';
    if (lower.includes('benign') && !lower.includes('likely')) return 'bg-emerald-100 text-emerald-700 border-emerald-200';
    if (lower.includes('likely benign') || lower === 'lb') return 'bg-green-100 text-green-700 border-green-200';
    if (lower.includes('vus') || lower.includes('uncertain')) return 'bg-amber-100 text-amber-700 border-amber-200';
    ...
    <div className="space-y-4">
      <Card className="bg-gradient-to-br from-white to-teal-50/50 border-teal-200 shadow-md">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <CardTitle className="text-xl font-bold text-slate-900">{currentData.name}</CardTitle>
              <CardDescription className="text-sm text-teal-600 font-medium">{currentData.fullName}</CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-slate-600 leading-relaxed line-clamp-3">{currentData.description}</p>

          <div className="grid grid-cols-3 gap-3">
            <div className="text-center p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
              <div className="text-xs text-teal-600 font-semibold mb-1">Chr</div>
              <div className="text-lg font-bold text-slate-900">{currentData.chromosome}</div>
            </div>
            <div className="text-center p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
              <div className="text-xs text-teal-600 font-semibold mb-1">Position</div>
              <div className="text-sm font-mono font-medium text-slate-900">
                {currentData.start.toLocaleString()}
              </div>
            </div>
            <div className="text-center p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
              <div className="text-xs text-teal-600 font-semibold mb-1">Length</div>
              <div className="text-lg font-bold text-slate-900">{geneLength} bp</div>
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-2">
            <a 
              ...
              className="flex-1"
            >
              <Button size="sm" variant="outline" className="w-full h-8 border-teal-200 text-teal-700 hover:bg-teal-50">
        ...
        {hoveredNucleotide ? (
            ...
            <CardHeader className="pb-3 bg-gradient-to-r from-slate-50 to-teal-50/30 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className="p-3 bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl shadow-md">
                  <span className="text-xl font-bold text-white">{hoveredNucleotide.base}</span>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <CardDescription className="text-sm text-slate-600">
                    Genomic: {(currentData.strand === '-' ?
                      currentData.end - hoveredNucleotide.id + 1 :
                      currentData.start + hoveredNucleotide.id - 1).toLocaleString()}
                  </CardDescription>
                </div>
                <Badge variant="outline" className="bg-white text-slate-700 border-slate-300 font-mono text-base px-3 py-1">
                ...
                if (totalVariants === 0) {
                  return (
                    <div className="flex flex-col items-center justify-center py-10 text-center">
                      <svg className="w-14 h-14 mb-4 text-teal-400" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="28" cy="28" r="24" stroke="currentColor" strokeWidth="2"/>
                        <path d="M20 28l4 4 12-12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                      <div className="text-base font-semibold text-slate-700 mb-1">No known variants</div>
                      <div className="text-sm text-slate-500">This position appears conserved</div>
                    </div>
                  );
                }
                      ...
                      <div className="p-4 bg-gradient-to-br from-red-50/50 to-white rounded-xl border border-red-200 shadow-sm">
                        ...
                        <div className="space-y-3">
                          {variantInfo.clinicalVariants.slice(0, 3).map((variant: Variant, index: number) => {
                            ...
                            return (
                              <div key={index} className="p-3 bg-white rounded-lg border border-red-100 shadow-sm">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Badge className={`text-xs font-medium ${getClinicalBadgeClass(variant.clinical_significance)}`}>
                                  ...
                                  <div className="mb-2 p-2 bg-purple-50/50 rounded-lg border border-purple-200">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {linkedVariants.map((linked, linkedIndex) => (
                                        <span key={linkedIndex} className="text-xs bg-white px-2 py-0.5 rounded border border-purple-200 text-slate-700">
                                          <span className="font-mono">{normalizeVariantId(linked.id)}</span>
                                          <span className="mx-1 text-purple-400">·</span>
                                          <span>{linked.ref}→{linked.alt}</span>
                                        </span>
                                      ))}
                                    </div>
                                  </div>
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 pt-2 border-t border-slate-100">
                                  {variant.gnomad_ac !== undefined && variant.gnomad_ac > 0 && (
                                    <span className="text-xs text-slate-600">
                                      <span className="font-semibold text-slate-700">gnomAD:</span> {variant.gnomad_ac.toLocaleString()}
                                    </span>
                                  )}
                                  {variant.aou_ac !== undefined && variant.aou_ac > 0 && (
                                    <span className="text-xs text-slate-600">
                                      <span className="font-semibold text-slate-700">AoU:</span> {variant.aou_ac.toLocaleString()}
                                    </span>
                                  )}
                                  {variant.ukbb_ac !== undefined && variant.ukbb_ac > 0 && (
                                    <span className="text-xs text-slate-600">
                                      <span className="font-semibold text-slate-700">UKBB:</span> {variant.ukbb_ac.toLocaleString()}
                                    </span>
                                  )}
                                </div>
                                  ...
                                  <div className="mt-3 pt-2 border-t border-slate-100">
                                    ...
                                    <div className="space-y-1.5">
                                      {variantLiterature.map((lit, litIndex) => (
                                        <div key={litIndex} className="flex items-start justify-between gap-2 text-xs bg-slate-50 p-2 rounded border border-slate-100">
                                          <div className="min-w-0 flex-1">
                                            <span className="text-slate-700 font-medium truncate block">{lit.authors}</span>
                                            <span className="text-slate-500">({lit.year}) · {lit.count} individual{lit.count !== 1 ? 's' : ''}</span>
                                          </div>
                                          <a 
                      ...
                      <div className="p-4 bg-gradient-to-br from-blue-50/50 to-white rounded-xl border border-blue-200 shadow-sm">
                        ...
                        <div className="space-y-2">
                          {variantInfo.populationVariants.slice(0, 3).map((variant: Variant, index: number) => {
                            return (
                              <div key={index} className="p-3 bg-white rounded-lg border border-blue-100 shadow-sm">
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex items-center gap-2">
                                    <div className="flex flex-col">
                                      <button 
          ...
          <>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg font-bold text-slate-900">Nucleotide Details</CardTitle>
              <CardDescription className="text-sm text-slate-600">
            ...
            <CardContent>
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <svg className="w-16 h-16 mb-4 text-slate-300" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="text-base font-semibold text-slate-700 mb-1">No nucleotide selected</div>
                <div className="text-sm text-slate-500">Hover over a nucleotide to view variant information</div>
              </div>
            </CardContent>
          </>
        )}
