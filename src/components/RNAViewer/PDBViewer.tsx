...
const PDBViewer: React.FC<PDBViewerProps> = ({
  ...
  const rnaChainInfoRef = useRef<Map<
    string,
    { minRes: number; maxRes: number; count: number }
  > | null>(null); // Cache chain info

  // Color mapping function for overlays
  const getOverlayColor = useCallback(
    (nucleotideId: number): string => {
        ...
        nglComponentRef.current.structure.eachResidue((residue: any) => {
          if (residue.isNucleic()) {
            ...
            if (!rnaChains!.has(chainName)) {
              rnaChains!.set(chainName, {
                minRes: residue.resno,
                maxRes: residue.resno,
                count: 0,
              });
            }
  ...
  function loadStructure() {
    ...
    type NGLStageType = {
      ...
      mouseControls: {
        add: (
          action: string,
          callback: (stage: any, pickingProxy: any) => void,
        ) => void;
      };
    };
    ...
    stage
      ...
      .then((component: NGLComponentType) => {
